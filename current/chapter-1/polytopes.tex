...
The {\em convex hull} of the points $z_i$ is an affine combination
with $\lambda_i\geq 0$ for all $i\in [m]$; we denote it as
\[
\conv \{z_i\ \mid\ i\in [m] \}=\{ \sum_i \lambda_i z_i\ \mid\ %
\lambda_i\geq 0,\ \sum_i \lambda_i=1 \text{ for } i\in [m] \}.
\]
A set of points $Z$ is {\em convex} if $Z=\conv(Z)$; it has
{\em dimension} $d$ if it has exactly $d + 1$ affinely independent points.
A convex set of $d+1$ points of dimension
...
its {\em dimension} is its dimension as convex hull.
...
that lie on a hyperplane of the form
...
The {\em polar} of the polytope $P$ is $P^\Delta$, where
\[
P=\{ x\in\reals^d\ \mid\ x\T c_i\leq 1,\ c_i\in\reals^d,\ i\in [k] \}
\]
\[
P^\Delta = \conv\{ c_i\ \mid\ i\in [k] \}.
\]
