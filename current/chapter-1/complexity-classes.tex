\section{Computational Problems and Complexity}\label{complexity-def-section}
...
an important open problem whether the inclusion is strict. If this were
not the case, it could be argued that there isn't any substantial
difference between finding a solution and verifying its validity.
This seems to contradict most intellectual experience, where the value
...
The class $\mathbf{TFNP}$ is equivalent
to $\mathbf{NP} \mathbf{co-NP}$ for function problems, and the existence
of a $\mathbf{TFNP}$-complete problem would imply
...
section \ref{ppad-section}.

The class $\mathrm{\mathbf{\# P}}$ is the class of all function
problems that output the number of possible certificates
