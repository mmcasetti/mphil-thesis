...
We start by recalling some standard definitions of computational complexity
theory; we then move on to the more recent classes {\bf TFNP} and {\bf PPAD},
first introduced in \cite{megiddo-papad} and \cite{ppad} respectively. The
latter, in particular, is a key concept in the study of the problems that
...
problem. Computational problems can be classified according to the form of their
...
{\em decision problems}, $y$ answers a ``{\sc Yes / No}'' question. The
{\em complement} of a decision problem $P$ is the problem $\bar{P}$
that returns ``{\sc No}'' for each instance of $P$ that returns
``{\sc Yes}'', and vice versa. {\em Search problems} are function problems
that return either an output $y$ such that $R(x,y)$, or ``{\sc No}'' if
it's not possible to find any such $y$. If $y$ is guaranteed to exist,
the problem is called a {\em total function problem}.
...
\todo[inline]{
, given by the {\em reducibility} from each
other.

deterministic Turing machines: here (we use only deterministic ones)}

Let $P_1$ be a computational problem, such that its instance $x$ is encoded
by $|x|$ bits. $P_1$ {\em reduces to the problem $P_2$ in polynomial time},
