...
proven using the argumet ``in any directed graph with one unbalanced node
...
The problems in {\sc End Of The Line} present a circuit $S$ (``successor''),
and a circuit $P$ (``predecessor'') that build a directed graph with an
edge $(x,y)$ if and only if $S(x)=y$ and $P(y)=x$; a {\em standard source}
$0^n$ is also given; the output is either a sink or a non-standard source.
Figure \ref{ppad-graph} presents an example of this.

\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=70ex]{chapter-1/fig/PPAD.pdf}%
\hfill\strut
\caption[The graph in a PPAD problem]{%
