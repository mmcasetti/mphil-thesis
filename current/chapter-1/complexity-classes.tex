...
The latter can be ``left'', ``right'', or ``stay'', represented respectively
by $\leftarrow,\rightarrow,-$. The first symbol is never overwritten,
and from there the cursor can only go left.

In some cases, after some finite time, the machine reaches the state
{\sc Yes} (the machine {\em accepts} the input), {\sc No} (the machine
{\em rejects} the input) or the {\em halting state} $h$. The output
...
Formally, $\mathcal{M}=(K,\Sigma,\delta,s)$, where $K$ and $\Sigma$ are
finite, $\Sigma\cap K=\varnothing$ and $\Sigma$ always contains the symbols
$\sqcup$ and $\rhd$.
...
the output in the tape of the $k$-th string (assuming the machine halts).
...
languages $L=\{ x\ |\ R(x,y) \text{ holds for some } y \}$ such that the
binary relation $R(x,y)$ on strings satisfies the following conditions:
\begin{enumerate}
...
Notice that there may be many different certificates for each solution
of a problem. Consider for instance the problem {\sc Hamilton Path},
defined as ``given an input graph $x$, is it possible to find a Hamiltonian
path $y$ of $x$?'' If a graph has more than one possible Hamiltonian path,
each one of these is a certificate for {\sc Hamilton Path}.

The class $\mathrm{\mathbf{\# P}}$ is the class of all problems that output
the number of possible certificates
% problems in NP are decision problems: solutions are either Y or N;
% the number of possible certificates for the solution of an instance
% is the important thing
for a problem in $\mathrm{\mathbf{NP}}$.
Formally: the {\em counting problem} associated with a binary
relation $Q(x,y)$ is ``given $x$, how many $y$ are there such
that $(x,y)\in Q$?'' Then $\mathrm{\mathbf{\# P}}$ is the class of
all counting problems associated with binary relations that are both
polynomially balanced and polynomially decidable.
It is interesting to notice that there are $\mathrm{\mathbf{\# P}}$-complete
problems for which the corresponding search problem can
be solved in polynomial time. An example close to the topic
...
Finally, the class {\bf PSPACE} is the set of decision problems for which
the solution by a Turing machine can be found using a tape that has length
...
% importance P ?= NP and NP ?= co-NP
% convetional view that these equalities do not hold
% mention here the Megiddo-Papadmitriou result:
%% if there is a TFNP-hard problem, then NP = co-NP (check!)

It is also still unknown if
\mbox{{\bf NP = co-NP}}.
