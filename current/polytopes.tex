...
such that $b_j\T x = v$. These points represent the strategies $x\in X$ of
player 1 that give exactly payoff $v$ to player 2 when he plays strategy $j$.
The projection of the facet defined by $b_j\T x = v$ to $X$ will have
...
of the form $(U,C\T)$, where the columns of the matrix $U$ are unit vectors.
The form of the theorem that we give here follows the notation in Savani
and von Stengel \cite{svs-15}.

\begin{theorem}\label{unit-vect-thm}
Let $l:[n]\to [m]$, and let $(U,C\T)$ be the unit vector game where
...
where $N_i = \{ j\in [n] | l(j)=i \}$ for $i\in [m]$, and

\begin{equation}
P^l = \{ x\in\reals^m | x\geq\0,\ C\T x\leq\1 \}
\end{equation}

Label the $m + n$ inequalities in the definition of $P^l$ as follows,
and label every facet of $P^l$ according to the inequality defining it.

\begin{itemize}
\item $x_i\geq 0$ has label $i$, for $i\in [m]$
\item $c_j\T x \leq 1$ has label $l(j)$, for $j\in [n]$
\end{itemize}
...
the pair $(x,y)$ is a Nash equilibrium of $(U,C\T)$
\end{theorem}
