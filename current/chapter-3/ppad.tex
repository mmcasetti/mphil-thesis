\section{Polynomial Parity Argument}\label{ppad-section}

As mentioned in section \ref{complexity-def-section},
Megiddo and Papadimitriou \cite{megiddo-papad} have proved that
it is impossible to have a complete problem for the class
$\mathrm{\mathbf{TFNP}}$ ({\em total function non-deterministic polynomial-time})
unless $\mathrm{\mathbf{NP}}=\mathrm{\mathbf{co-NP}}$,
which is usually conjectured to be false.
To circumvent this limitation, Papadimitriou \cite{ppad} focused
on the argument that proves the solution of the problems in {\bf TFNP},
introducing, among others, the classes
$\mathrm{\mathbf{PPA}}$ ({\em Polynomial Parity Argument}) and
$\mathrm{\mathbf{PPAD}}$ ({\em Polynomial Parity Argument, Directed version}).
...
in terms of polynomial-time Turing machines instead of the
polynomial-sized circuits.

\begin{problem}
{End Of The Line}
{Two polynomial-sized circuits $S$ and $P$ with $n$ input bits and $n$ output bits such that
$P(0^n)=0^n\neq S(0^n)$.}
