...
is {\em almost completely labeled} if all labels appear
once in $x$ except for one {\em missing label} $k\in [m]$ and one
{\em duplicate label} $h\in [m]$ that appears twice; formally,
if there is exactly one $k\in [m]$ such that
\begin{equation}
\label{acl-equation}
\{ j\in [m]\ \mid\ l(x_i)=j\text{ for some }i\in [n] \}= [m]\setminus \{ k \}.
\end{equation}
...
labeled vertices with missing
\linebreak[5]
label $k$; these vertices can be seen as
...
the problem {\sc Another Completely Labeled}
\linebreak[4]
{\sc Vertex}.
...
there are no ``loops'' where a vertex is visited more than once.
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=75ex]{chapter-3/fig-lh/lh.pdf}%
\hfill\strut
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=45ex]{chapter-3/fig-lh/shapley-game-sym.pdf}%
\hfill\strut
\caption[A game with disjoint Lemke paths]{%
Lemke paths on the best response polytope of game (\ref{disj-lp-game}).
}
...
and of
\linebreak[5]
Proposition \ref{lh-works-ppa-thm} is quite straightforward.
\begin{algorithm}
\SetKwInOut{Input}{input}
\SetKwInOut{Output}{output}
\Input{
A simplicial $m$-polytope $Q$ with $n$ vertices and a
labeling $l_v:[n]\to [d]$ of the vertices of $P$.
...
Algorithm (\ref{lh-alg}) to the polytope
\linebreak[5]
$P^l= \{ x\in\reals^m\ \mid\ x\geq\0,\ B\T x\leq\1 \}$ in (\ref{p-l}),
or the Dual Lemke-Howson
\linebreak[3]
Algorithm (\ref{lh-dual-alg}) to the polytope
$Q=\conv(\{ e_1,\ldots,e_m \})\ \cup\ \{ c_1,\ldots,c_n \})$
\linebreak[4]
in (\ref{p-l-dual}).
...
Q = \{ y\in\reals^n\ \mid\ y\geq\0,\ A y\leq\1 \}; \\
P^l= \{ x\in\reals^m\ \mid\ x\geq\0,\ B\T x\leq\1 \}
\end{eqnarray*}
as in (\ref{br-polytopes}) and (\ref{p-l}).
...
{
pivot on the duplicate label $h$ to $s'\neq s_0$ \\
rename $s_0=s$, $s=s'$
}
\Return $s$
...
\begin{figure}[ht]
\strut\hfill
\includegraphics[width=55ex]{chapter-3/fig-lh/123432-lh-pivot.pdf}%
\hfill
\small
...
We will now show a construction that we will allow us to extend the
analogous of Proposition \ref{lh-works-ppa-thm} for the Lemke-Howson for Gale
Algorithm to prove the {\bf PPAD} complexity of \anothergale.
A {\em permutation} of the elements of an ordered set $S$ is a sequence
without repetition of elements of $S$; a {\em transposition} is a permutation
of exactly two elements of $S$.
The {\em sign of a permutation} $\sigma$ is
$\sign(\sigma)=(-1)^m$, where $m$ is the number of transpositions needed
to get $\sigma_0=1\ldots n$ from $\sigma$.
...
Consider now the steps of the Lemke paths in the Lemke-Howson for Gale
Algorithm; we assume that $\sign(s_0)=+1$ without loss of generality, since
the negative case is the same just with opposite signs.
...
Gale string, we have that $\sign(l_1)=-1$, therefore $\sign(l_2)=+1$.
The next pivoting step drops the label $h$ from index $i_2$, so again we
change sign.
Running the Lemke-Howson for Gale Algorithm will therefore result in the sign
of the completely labeled Gale strings swinging as in
Table \ref{lhg-sign-figure}.
...
A result similar to Proposition \ref{lhg-works-ppad-thm}, but more general,
is given in Shapley \cite{shapley}: two Nash equilibria at the ends of a
Lemke path have opposite {\em index}, which is analogous to the sign but
defined using determinants on the payoff matrices for the equilibrium
support; the artificial equilibrium is assigned index $+1$.
This is used to prove that a nondegenerate game with $n$ Nash equilibria
...
\begin{figure}[h]
\strut\hfill
\includegraphics[width=60ex]{chapter-3/fig-lh/pivoting-sign-example.pdf}%
\hfill\strut
...
An example of labeling $l:[2d]\to [d]$ where the length of the
Lemke paths on the cyclic polytope $C_d(2d)$, therefore on $G(d,2d)$,
grows exponentially in $d$, regardless of the label that is dropped,
is given by Morris \cite{morris}. This labeling is given for $d$ both
...
\end{eqnarray*}
Avoiding repetitions and restricting to the case of $d$ even, as we assumed,
the Morris' labeling is simplified to $l:[2d-2]\to [d]$ as follows:
\begin{eqnarray*}
l(k)=k          & \quad\quad\text{for }k\in [d];    \\
...
motivation to study the compelxity of \anothergale.
