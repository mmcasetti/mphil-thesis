...
The {\bf PPA} complexity of \anothergale\ follows from an argument
similar to Proposition \ref{lh-works-ppa-thm}. We will extend the result
to prove its {\bf PPAD} complexity.
...
$s(i_2)=\1$ before the last pivot (the ``old'' position of
\linebreak[4]
the \1).
...
the Lemke-Howson for Gale Algorithm results in the sign
...
Lemke paths on the cyclic polytope $C_d(2d)$ for the Lemke-Howson
Algorithm (\ref{lh-alg}), therefore on $G(d,2d)$ for the Lemke-Howson for
Gale Algorithm (\ref{lhg-alg}),
grow exponentially in $d$ regardless of the label that is dropped
was given by Morris \cite{morris}. This labeling is given for $d$ both
...
\end{eqnarray*}
Avoiding repetitions and restricting to the case of $d$ even
...
Table \ref{morris-6-fig} shows the Lemke path for label 1.
\begin{table}[hbt]
\begin{center}
\large
...
{The Lemke path for the Morris labeling on $G(4,6)$ dropping label 1.}
\label{morris-6-fig}
\end{table}
\end{example}
