...
$E=E\cup\{ (l(n),l(1)) \}$ \\
Set $s=0^n$ \\
Run the Edmonds's Algorithm on $G$ \\
\If{ The Edmonds's Algorithm returns a perfect matching $M$ of $G$ }
    {
    \For{ $(l(i),l(j))\in M$ }
        {
        $s(i)=s(j)=\1$
...
\For{ $m\in M_0$ }
    {
    $E_m=E\setminus\{ m \}$ \\
    Run the Edmonds's Algorithm on $G_m=(V,E_m)$ \\
    \If{ The Edmonds's Algorithm returns a perfect matching $M_m$ of $G_m$ }
        {
        \For{ $(l(i),l(j))\in M_m$ }
            {
            $s(i)=s(j)=\1$
...
of the game from a cyclic polytope is known.
