...
apply the algorithm detailed above to the corresponding $l':[n+1]\to [d+1]$
to find a completely labeled Gale string $s'$, and from that a
Gale string $s$ that is completely labeled for $l$.
...
\If{ $d$ odd }
    {
    Set ``$d$ originally odd'' = True
    Set $d = d+1$, $n = n+1$ \\
    Set $l':[n+1]\to [d+1]$, with $l'(i)= l(i)$ for $i\in [n]$, $l(n+1) = d+1$ \\
    Set $l = l'$
    }
\Else{
    Set ``$d$ originally odd'' = False
    }
...
The matching $M_1$ (in blue) corresponds to the completely labeled
Gale strings $s_1=\1\1\1\1\1\1 0000$ and the matching $M_2$ (in red)
corresponds to the string $s_1=\1 0000\1\1\1\1\1$.
}
\label{perfect-matching-fig}
\end{figure}
\end{example}

\todo[inline]{done until here}
