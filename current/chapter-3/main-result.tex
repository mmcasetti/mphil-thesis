...
\linebreak[4]
accessory problem \gale\ can be solved in polynomial time.
...
\begin{proof}
Recall that we consider the Gale strings $s\in G(d,n)$ as ``loops,''
so $s(n+i)=s(i)$ and $l(n+i)=l(i)$.

Let $G=(V,E)$ be the multigraph with $V=[d]$, so that the vertices of $G$
correspond to the labels $l(i)\in [d]$, and
\begin{equation}
\label{edges-graph}
E=\{(l(i),l(i+1))\text{ for }i\in[n] \},
\end{equation}
...
pairs of indices $(i,i+1)$ with $s(i)=s(i+1)=1$.
Let
\begin{equation}
\label{matching-graph}
M=\{ (l(i),l(i+1))\ \mid\ s(i)=s(i+1)=1\text{ without repetitions of } i \}.
\end{equation}
Since $s$ is completely labeled, all labels $l(i)\in [d]$ occur at exactly
one of the $k/2$ indices, therefore as endpoints of edges in $M$;
since the there are no repetitions of $i$'s in $M$, there is only one such
edge. Hence, $M$ is a perfect matching of $G$.

Conversely, let $M$ be a perfect matching for $G=(V,E)$ as
in (\ref{matching-graph}).
...
Two perfect matchings for the Morris labeling $l=1234564523$.
}
...
\linebreak
$l:[n]\to [d]$ with $V=[d]$ and $E$ as in (\ref{edges-graph}); let $M_0$ be
the perfect matching of $G$ corresponding to the completely
...
\hfill\strut
\caption[A second perfect matching]{%
...
Right: The set $M'=\{ e_6,e_8,e_{10} \}$ is a perfect matching of both $G_1$
and $G$.
}
