...
ENDM (2010). Our result shows a polynomial time algorithm to
find a Nash equilibrium for a particular class of games,
which was previously used by Savani and von Stengel (2006)
as an example of exponential time for the classical
Lemke-Howson algorithm for bimatrix games (1964).
...
% In section \ref{galestrings} we define {\em Gale string} and the problems \gale and \anothergale. In section \ref{games} we see how these problems are connected to the problem \textsc{Nash} for a special case of games. In section \ref{lh} we give the context of our result: we show how \anothergale belongs to the class $\mathbf{PPAD}$ and we relate the result in \ref{svs} about a possible $\mathbf{PPAD}$-completeness of the problems we examine. Finally, in section \ref{main} we give our result, proving that both \gale and \anothergale are polynomially solvable, and we consider the effect this has on the conjecture of \ref{svs}. Further results stemmed from our theorem are reported in section \ref{final}.

An appendix collects the main notation used throughout the thesis.

A final appendix presents our correction to an error in the proof of the $\mathbf{PPAD}$-completeness of \textsc{Nash} in \ref{dgp}.
