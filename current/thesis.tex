...
\subsection{The Lemke-Howson Algorithm and the Class PPAD}
% section? subsection? needed?
...
% parity. LHG. It would be better not in the proof, so LHG is more visible

% Through a ``parity argument'' we can show the the following property of

Gale strings


\begin{definition}{pivoting}
Let $l$ be a string of labels of length~$n$, let $s$ be a corresponding
completely or almost completely labeled Gale string, and let $i \in [n]$ be
such that $s(i)=1$. Consider the strings as ``loops'' by identifying position
$i$ with any position $i+kn$.

We define as {\em pivoting} the following operation:
\begin{algorithm}
...
{ set $s(i+j+1)=1$ }
\If{ the odd maximal run of $1$s is on the \textbf{left} of position $i$ }
{ set $s(i-j-1)=1$ }
\caption{Pivoting}
\end{algorithm}
\end{definition}

Note that the existence of the odd maximal run is guaranteed by the Gale
evenness condition.



\begin{algorithm}\label{lhg}
\SetKwInOut{Input}{input}
\SetKwInOut{Output}{output}

\Input{A $n$-string $l \in \left\{1,\ldots,d\right\}^{n}$ where $d$ is even
and $d< n$, a Gale string $s$ associated with $l$.}
\Output{A Gale string $s'\neq s$ associated with $l$.}

\BlankLine

pivot any label of s\\
%pick $i \in [n]$ s.t. $s(i) = 1$ and pivot $l(i)$;\\
...
Using algorithm \ref{lhg} we can show a fundamental property of Gale
strings.
...
It holds for odd $d$ as well but we assume throughout that $d$ is even.
