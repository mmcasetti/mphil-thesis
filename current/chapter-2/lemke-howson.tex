...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=27ex]{chapter-2/fig/lh.pdf}%
\hfill\strut
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=27ex]{chapter-2/fig/octahedron-lhdual.pdf}%
\hfill\strut
\caption{%
...
In the case of unit vector games, theorems \ref{unit-vector-thm} and
\ref{unit-vector-dual-thm} imply that it is enough to apply
algorithm \ref{lh-alg} to the polytope
$P^l= \{ x\in\reals^m\ |\ x\geq\0,\ B\T x\leq\1 \}$ in (\ref{p-l-unitv}),
or the dual algorithm \ref{lh-dual-alg} to the polytope
$P^\Delta=\conv(\{ e_1,\ldots,e_m \})\ \cup\ \{ c_1,\ldots,c_n \})$ in
\ref{p-l-dual}.
This allows to find one Nash equilibrium by pivoting on $P^l$ or $P^\Delta$,
instead of considering the product of the best response polytopes
$P$ and $Q$.
The projection of a Lemke path for a missing label $i\in [m]$
on $P\times Q$ to $P$ defines a Lemke path in $P^l$;
\todo{how?}
however, $P\times Q$ has $m+n$ labels, therefore there could be
Lemke paths for a missing label $m+j$ with $j\in [n]$
on $P\times Q$ that are ``lost'' in the projection on $P^l$.
The following theorem, by Savani and von Stengel \cite{uvg},
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=27ex]{chapter-1/fig/123432-fac-name.pdf}%
\hfill\strut
\caption[Lemke-Howson for Gale algorithm and cyclic polytope]{%
Lemke-Howson for Gale algorithm and the corresponding facets of
the cyclic polytope.%
}
...
The analogous of propositions \ref{l-h-works-thm} and
\ref{l-h-dual-works-thm} is the following.
