...
Theorem~\ref{origgale-thm} to translate this to a problem on Gale strings.
First of all, we have to define a labeling for Gale strings such that a
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=50ex]{chapter-2/fig-gale-games/123432-fac-name.pdf}%
\hfill
\small
...
Let $s_0'(i)=s_0(i)$ for $i\in [n]$ and $s_0'(n+1)=1$, and let
$l':[n+1]\to[d+1]$ be
the labeling such that $l'(i)=l(i)$ for $i\in [n]$ and $l'(n+1)=d+1$.

Let $s'$ be a bitstring of length $n+1$ such that $s'(n+1)=1$, and let
$s$ be the bitstring of length $n$ such that $s(i)=s'(i)$ for $i\in [n]$.
First of all, notice that if $s'$ is a Gale string then $s$ is also a
Gale string: the Gale Evenness Condition holds in all the interior runs,
which is the only requirement for a Gale string in $G(d,n)$ when $d$ is odd.
Furthermore, if $s'$ is completely labeled for $l'$, then $s'(n+1)=1$,
since the only occurrence of label $d+1$ is at index $n+1$.
