...
$\overline{t_i}$'s such that for all the $t_k$'s such that
...
a facet in (\ref{facet-gs}) becomes the Gale evenness condition.
\end{proof}
\end{theorem}
...
and the choice of the $t_j$'s is irrelevant.
...
\begin{example}
As a counterexample, consider Figure \ref{notfacet-odd-hyper-fig}.
The points $t=t_4$ and $t'=t_6$ lie on the moment curve but are on opposite
sides of the hyperplane $H$.
The corresponding bitstring is  $s=\1 0\1 0\1 0$, which is
...
corresponding bitstring is $s=\1\1\10\10$.
\begin{figure}[hp]
\strut\hfill
...
Notice that it is not always possible to find a completely labeled Gale
string.
...
correspond exactly to facets of $Q$ that are completely labeled by $l_v$,
with the
\linebreak[5]
...
we will also assume that the labeling $l_s:[n]\to [d]$ satisfies
...
for the labeling shown in Figure \ref{c46-123432-fig}, left.
\begin{figure}[hbt]
\strut\hfill
