...
being the dual of a cyclic polytope. These can be represented
in a synthetic way using a combinatorial structure called Gale strings.
...
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=40ex]{chapter-2/fig-gale-def/cyc36.pdf}
\hfill\strut
...
\]
Notice that $0\10\1\1$ is a shift of $\10\1\10$ but it is not a Gale
string.
\end{example}
...
$t_j$'s in the definition of $C_d(n)$. Then the
$\mu_d(\overline{t_i})$'s define an hyperplane $H$ that
...
$t<\overline{t_i}<t'$ for one and only one $\overline{t_i}$ then
...
Notice that the moment curve has exactly $d$ zeroes implies
...
$s=\100\1\10$.
Figure \ref{cyc36fac-hyper-fig} shows the moment curve and its intersections
with the hyperplane $H$ in $\reals^3$ defined by the $t_i$ such that $s(i)=1$.
\begin{figure}[p]
\strut\hfill
\includegraphics[width=40ex]{chapter-2/fig-gale-def/cyc36fac.pdf}%
\hfill\strut
...
\begin{figure}[p]
\strut\hfill
\includegraphics[width=60ex]{chapter-2/fig-gale-def/100110-hyper.pdf}%
\hfill\strut
...
$s=\1 00 \1\1 0\in G(3,6)$ corresponds to the intersection of the moment
curve and the hyperplane where $\mu_3(t_1)=\mu_3(t_4)=\mu_3(t_5)=0$.
}
...
Figure \ref{cyc46fac-hyper-fig} shows  $s=\1\1\1\100$ as intersection with
the hyperplane $H$.
\begin{figure}[p]
\strut\hfill
...
the exterior facet, drawn in bold lines, corresponding to the Gale string
...
\begin{figure}[p]
\strut\hfill
\includegraphics[width=60ex]{chapter-2/fig-gale-def/111100-hyper.pdf}%
\hfill\strut
\caption[A facet of $C_4(6)$ as zeroes of the moment curve]{%
The facet of the cyclic polytope $C_4(6)$ corresponding to the Gale string
$s=\1\1\1\1 00\in G(4,6)$ corresponds to the intersection of the moment
curve and the hyperplane where
$\mu_4(t_1)=\mu_4(t_2)=\mu_4(t_3)=\mu_4(t_4)=0$.}
...
The points $t=t_4$ and $t'=t_6$ lie on the moment curve but are on opposite
...
There is a change of sign between two $t_j$'s; the bitstring $s=\1\1\1 0\1 0$
does not satisfy the Gale evenness condition, and the $t_j$'s do not
correspond to a facet of $C_4(6)$.
}
...
If the polytope $Q$ in Theorem \ref{unit-vector-dual-thm} is cyclic and we
...
The labels $l_s(i)=2,3,4$ appear only once in $l_s$, as
$l_s(2),l_s(4),l_s(6)$ respectively; therefore we must have
$s(2)=s(4)=s(6)=1$. For every other
$i\in [n]$ we have $l(i)=1$, so we have $l_s(i)=1$ for exactly one $i=1,3,5$.
...
$U=(e_{l(1),\ldots,e_{l(d)}})$ for some labeling $l:[n]\to [d]$ and
...
We will also assume that the labeling $l_s:[n]\to [d]$ is such
...
$s(i)$ and $s(i+1)$ can be equal to \1 (notice that it's possible that both
are 0s). So $s(i)s(i+1)$ will never be a run of even length that
``interferes'' with the Gale Evenness Condition, so we can
...
for the labeling shown in Figure \ref{c46-123432-fig}.
\begin{figure}[hbt]
\strut\hfill
\includegraphics[width=55ex]{chapter-2/fig-gale-def/123432-fac-name.pdf}%
\hfill
\small
...
The cyclic polytope $C_4(6)$; the labeling of the vertices correspond to
...
We can now define the problem \anothergale\ as in Table \ref{another-gale}.
...
$C_d(n)$ into the corresponding Gale strings in $G(d,n)$, following
the proof of Theorem \ref{gale-thm}.
