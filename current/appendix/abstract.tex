This thesis presents a report on original research, expanding on a result
\linebreak[5]
published as joint
work with Merschen and von Stengel in {\em Electronic Notes in Discrete
Mathematics}~\cite{main}. We presents a polynomial time
algorithm to solve two problems related to labeled Gale strings, a
combinatorial structure
\linebreak[5]
introduced by Gale in \cite{gale} that can be
...
complete for the class {\bf PPAD} (Polynomial Parity
Argument, Directed version, see Papadimitriou~\cite{ppad}). In turn, a major
motivation for the definition of PPAD was to study complementary pivoting
methods, including the Lemke-Howson algorithm.
...
Our proof exploits two results. First of all, as seen in Savani
and von Stengel~\cite{svs}~\cite{uvg}, we represent the Nash equilibria
of these special games as Gale strings.
Our original construction is a graph for which perfect matchings correspond
to Nash equilibria via Gale strings; exploiting the polynomial-time
algorithm for a perfect matching in a graph given by Edmonds~\cite{edm}
we have our result.
...
\linebreak[5]
Shapley~\cite{shapley}, in the framework of ``oiks''
as considered by
