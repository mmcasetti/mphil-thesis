...
a unit vector game $(U,B)$
\todo{not generic, via Savani-von Stengel \cite{uvg}?}
  where the best-response polytope $Q$ is
cyclic, exploiting theorems \label{unit-vector-dual-thm} and
\ref{cp-gs-gale-thm}.
...
Theorem \label{unit-vector-dual-thm} relies on a labeling of the
vertices $l_Q:[d+n]\to [d]$ defined as
\begin{itemize}
\item $l_Q(-e_i)=i$ for $i\in [d]$
\item $l_Q(c_j)=l_U(j)$ for the vertices $c_j\neq -e_i$, for $j\in [n]$
\end{itemize}
where $l_U:[n]\to [d]$ is the the labeling associated to the unit
vector game $(U,B)$.
We define the labeling $l:[d+n]\to [d]$ as follows:
\begin{itemize}
\item $l(i)=i$ for $i\in [d]$
\item $l(d+j)=l_U(j)$ for $j\in [n]$
\end{itemize}
The Gale strings $s\in G(d,d+n)$ that are completely labeled for $l$
...
Then the problem of finding a Nash equilibrium for a unit vector game
$(U,B)$ is equivalent to the problem \anothergale, defined as follows.
