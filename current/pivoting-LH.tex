...
pivot from $x_0$ to $x$ dropping label $k$ \\
...
Let $(U,B)$ be a unit vector game with dual cyclic best response polytope.
Let $l:[n]\to [d]$ be the labeling such that $U=(e_{l(1)}\cdots e_{l(d)})$.
In \ref{gs-labeling-unitv}, we have given a labeling $l_s:[n+d]\to [d]$;
in this labeling the ``artificial'' equilibrium corresponded to the
